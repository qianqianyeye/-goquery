...
//		return url.Parse("http://122.241.73.245:1133")//根据定义Proxy func(*Request) (*url.URL, error)这里要返回url.URL
//	}
//	transport := &http.Transport{Proxy: proxy}
//	client := &http.Client{Transport: transport}
//	resp, err := client.Get("http://www.avxsl5.com/") //请求并获取到对象,使用代理
//	if err != nil {
//		log.Print("a")
//		log.Fatal(err)
//	}
...
//	resp.Body.Close()
//	//getIp("local")
//	//getIp("http://190.249.148.149:47349")
//}
...
func getIp(ip string){
	...
	for i := 1; i <= PAGE; i++ {
		...
		response,_ := GetRep(xici + strconv.Itoa(i),ip)
...
* 返回response
*/
func GetRep(urls string,ip string) (*http.Response,error ){
	...
	//fmt.Printf("使用代理:%s\n",proxy)
	proxy := func(_ *http.Request) (*url.URL, error) {
		return url.Parse(ip)//根据定义Proxy func(*Request) (*url.URL, error)这里要返回url.URL
	}
	transport := &http.Transport{Proxy: proxy}
	client := &http.Client{Transport: transport}
	response, err := client.Get(urls) //请求并获取到对象,使用代理
	fmt.Println(&response)
	if err != nil {
		log.Fatal(err)
	}
	//data, err := ioutil.ReadAll(response.Body) //取出主体的内容
	//if err != nil {
	//	log.Fatal(err)
	//}
	//fmt.Printf("%s",string(data)) //打印
	//fmt.Println(response.Body)
	if err != nil || response.StatusCode != 200{
		fmt.Println(err)
		if response.StatusCode== 404 {
			return response,errors.New("404 error")
		}
		fmt.Printf("line-129:遇到了错误-并切换ip %s\n",err)
...
* 返回response
*/
func GetResponse(urls string,ip string) *http.Response {
	proxy, err := url.Parse(ip)
	fmt.Println(err)
	client := &http.Client{
